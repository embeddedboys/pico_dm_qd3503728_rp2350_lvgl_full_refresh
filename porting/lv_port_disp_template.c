...
#define BYTE_PER_PIXEL             \
	(LV_COLOR_FORMAT_GET_SIZE( \
		LV_COLOR_FORMAT_RGB565)) /*will be 2 for RGB565 */
...
void lv_port_disp_init(void)
{
     ...
     * -----------------------------------*/
	lv_display_t *disp =
		lv_display_create(MY_DISP_HOR_RES, MY_DISP_VER_RES);
...
// #define MY_DISP_BUF_SIZE    (MY_DISP_HOR_RES * 10)
	...
	static uint16_t buf_1_1[MY_DISP_BUF_SIZE];
		       ...
		       uint8_t *px_map)
{
	if (disp_flush_enabled) {
		ili9488_video_flush(area->x1, area->y1, area->x2, area->y2,
				    (void *)px_map, lv_area_get_size(area));
	}
